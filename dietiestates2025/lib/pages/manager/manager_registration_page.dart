...
class ManagerRegistrationPage extends StatefulWidget {
  ...
  @override
  _ManagerRegistrationPageState createState() =>
...
class _ManagerRegistrationPageState extends State<ManagerRegistrationPage> {
  ...
  // Funzione per registrare il manager
  Future<void> _registerManager() async {
    ...
    Utente newManager = Utente(
      username: username,
      password: password,
      nome: nomeCapitalized,
      cognome: cognomeCapitalized,
      email: email,
      idRuolo: 3,  // idRuolo per il manager
    );

    try {
      await _utenteRepository.createUtente(newManager);
      _showSuccessDialog('Registrazione dell\'agente avvenuta con successo!');
      _clearFields();
    } catch (e) {
      _showErrorDialog(
          'Errore durante la registrazione: Il manager è già registrato');
    } finally {
      setState(() => _isLoading = false);
    }
  }
          ...
          actions: [
            CupertinoDialogAction(
              ...
              onPressed: () {
                ...
                ); // Naviga alla pagina ManagerHomePage
              },
              child: const Text('OK', style: TextStyle(color: Color(0xFF0079BB))),
            ),
          ],
